			...
			<el-card class="box-card" v-for="(guest, index) in guestPermissions" :key="guest.userId">
				...
				<el-form label-position="top">
					<el-form-item label="Allow this person to view records of this Table">
						<el-radio-group v-model="guest.role.read.allowed">
							<el-radio :label="true">Yes</el-radio>
							<el-radio :label="false">No</el-radio>
						</el-radio-group>
					</el-form-item>
					<el-form-item label="Allow this person to ADD record to this table">
						<el-radio-group v-model="guest.role.add.allowed" style="margin-right:30px">
							<el-radio :label="true">Yes</el-radio>
							<el-radio :label="false">No</el-radio>
						</el-radio-group>
						<el-checkbox v-model="guest.role.add.approval" :disabled="!guest.role.add.allowed">You have to <u>approve</u> first, after that the record will be added</el-checkbox>
					</el-form-item>
					<el-form-item label="Allow this person to MODIFY records of this table">
						<el-radio-group v-model="guest.role.update.allowed" style="margin-right:30px">>
							<el-radio :label="true">Yes</el-radio>
							<el-radio :label="false">No</el-radio>
						</el-radio-group>
						<el-checkbox v-model="guest.role.update.approval" :disabled="!guest.role.update.allowed">You have to <u>approve</u> first, after that the record will be modified</el-checkbox>
					</el-form-item>
					<el-form-item label="Allow this person to REMOVE records from this table">
						<el-radio-group v-model="guest.role.delete.allowed" style="margin-right:30px">>
							<el-radio :label="true">Yes</el-radio>
							<el-radio :label="false">No</el-radio>
						</el-radio-group>
						<el-checkbox v-model="guest.role.delete.approval" :disabled="!guest.role.delete.allowed">You have to <u>approve</u> first, after that the record will be deleted</el-checkbox>
					</el-form-item>
				</el-form>
			</el-card>
		</el-row>
	...
	module.exports = {
		...
		methods: {
			...
			removeAdmin(index, row) {
				this.$confirm(`Are you sure want to remove '${row.email} from admin list' ?`, 'Warning', {
					confirmButtonText: 'OK',
					cancelButtonText: 'Cancel',
					type: 'warning'
				}).then(() => {
