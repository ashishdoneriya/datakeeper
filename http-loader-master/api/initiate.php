...
//public role = none/user/contributorRW/contributorR/contributorW
...
//role = administrator / contributorRW / contributorR / contributorW / user
...
// Creating table 'write_requests'
$query = "create table if not exists write_requests (reqId int primary key not null auto_increment, userId int not null, tableName varchar(30) not null, fields text not null, foreign key (userId) references users(userId), foreign key (refreeId) references users(userId))";
$db->query($query);
